...
class TaskTest {
    ...
    class Describe_equals {
        @Nested
        @DisplayName("값이 같은 task가 주어진다면")
        class Context_with_same_task {
            Task sameTask;

            @BeforeEach
            void setUp() {
                sameTask = new Task();
                sameTask.setTitle(TASK_TITLE);
                sameTask.setId(TASK_ID);
            }
        ...
        @Nested
        @DisplayName("값이 다른 task가 주어진다면")
        class Context_with_different_task {
            Task differentTask;

            @BeforeEach
            void setUp() {
                differentTask = new Task();
                differentTask.setTitle(NEW_TASK_TITLE);
                differentTask.setId(NEW_TASK_ID);
            }
